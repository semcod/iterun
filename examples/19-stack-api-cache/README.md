...
Dwa serwisy: FastAPI (`18090`) + gotowy obraz `redis:7-alpine` (bez własnego Dockerfile).

```bash
./run.sh
ITERUN_RUNTIME=pactown ./run.sh
```
...
| Element | Wartość |
|---------|---------|
| Intent (`INTENT.name`) | `api-cache-stack` |
...
# HTTP
curl -s http://localhost:18090/cache/status

# Docker
docker compose -f generated/docker-compose.yaml -p intent-api-cache-stack ps

# logi kontenera
docker logs -f $(docker ps -q --filter name=api-cache-stack | head -1) 2>/dev/null || \
